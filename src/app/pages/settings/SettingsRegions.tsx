...
import { ChevronRight, Loader2, MapPin, Pencil, Plus, RefreshCw, Trash2 } from 'lucide-react';
import {
  createAdminRegion,
  deleteAdminRegion,
...
export function SettingsRegions() {
  ...
  return (
    <SettingsSectionChrome
      title="Viloyatlar"
      description="Viloyat ustiga bosing — tumanlar sahifasi ochiladi"
      actions={
              ...
              <tbody className="divide-y divide-border">
                {filtered.length === 0 ? (
                  ...
                  filtered.map((row) => (
                    <tr
                      key={row.id}
                      className={`${rowElite} cursor-pointer hover:bg-muted/40`}
                      onClick={() => navigate(`${SETTINGS_BASE}/regions/${row.id}`)}
                    >
